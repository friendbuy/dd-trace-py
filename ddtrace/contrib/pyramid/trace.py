...
def includeme(config):
    ...
    if not isinstance(pyramid.renderers.RendererHelper.render, wrapt.ObjectProxy):
        wrapt.wrap_function_wrapper(
            "pyramid.renderers", "RendererHelper.render", trace_render
        )
...
def trace_tween_factory(handler, registry):
    ...
    tracer.set_service_info(service=service, app="pyramid", app_type=AppTypes.web)
    ...
    if enabled:
        ...
        def trace_tween(request):
            ...
            with tracer.trace(
                "pyramid.request", service=service, resource="404"
            ) as span:
                ...
                try:
                    ...
                    if request.matched_route:
                        span.resource = "{} {}".format(
                            request.method, request.matched_route.name
                        )
