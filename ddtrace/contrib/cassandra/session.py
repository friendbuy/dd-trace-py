...
def _close_span_on_success(result, future):
    ...
    try:
        span.set_tags(
            _extract_result_metas(cassandra.cluster.ResultSet(future, result))
        )
...
def _close_span_on_error(exc, future):
    ...
    try:
        ...
        log.debug(
            "traced_set_final_exception was not able to set the error, failed with error: %s",
            e,
        )
    finally:
        span.finish()
...
def traced_execute_async(func, instance, args, kwargs):
    ...
    try:
        ...
        setattr(
            result,
            "_set_final_result",
            wrapt.FunctionWrapper(result._set_final_result, traced_set_final_result),
        )
        setattr(
            result,
