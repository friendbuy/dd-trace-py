...
class RateSamplerTest(unittest.TestCase):
    def test_sample_rate_deviation(self):
        for sample_rate in [0.1, 0.25, 0.5, 1]:
            ...
            deviation = abs(len(samples) - (iterations * sample_rate)) / (
...
class RateByServiceSamplerTest(unittest.TestCase):
    ...
    def test_sample_rate_deviation(self):
        for sample_rate in [0.1, 0.25, 0.5, 1]:
            ...
            assert (
                writer != tracer.writer
            ), "writer should have been updated by configure"
            ...
            # Less than 2% deviation when "enough" iterations (arbitrary, just check if it converges)
            deviation = abs(samples_with_high_priority - (iterations * sample_rate)) / (
                iterations * sample_rate
            )
            assert deviation < 0.02, "Deviation too high %f with sample_rate %f" % (
                deviation,
