...
class TestWorkers(TestCase):
    ...
    def test_worker_http_error_logging(self):
        ...
        ok_(
            "failed_to_send traces to Agent: HTTP error status 400, reason Bad Request, message Content-Type:"
            in logged_errors[0]
        )
...
class TestAPITransport(TestCase):
    ...
    def test_send_presampler_headers_not_in_services(self, mocked_http):
        ...
        services = [{"client.service": {"app": "django", "app_type": "web"}}]
    ...
    def test_send_service_called_multiple_times(self):
        ...
        services = [
            {
                "backend": {"app": "django", "app_type": "web"},
                "database": {"app": "postgres", "app_type": "db"},
            }
        ]
...
class TestAPIDowngrade(TestCase):
    ...
    @skip(
        "msgpack package split breaks this test; it works for newer version of msgpack"
    )
