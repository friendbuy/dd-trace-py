...
class DjangoAutopatchTest(DjangoTraceTestCase):
    ...
    def test_autopatching_twice_middleware_classes(self):
        ...
        found_mw = settings.MIDDLEWARE_CLASSES.count(
    ...
    def test_autopatching_middleware(self):
        ...
        # exist at all in 2.0.
        ok_(
            not getattr(settings, "MIDDLEWARE_CLASSES", None)
            or "ddtrace.contrib.django.TraceMiddleware"
            not in settings.MIDDLEWARE_CLASSES
        )
        eq_(settings.MIDDLEWARE[-1], "ddtrace.contrib.django.TraceExceptionMiddleware")
        ok_(
            not getattr(settings, "MIDDLEWARE_CLASSES", None)
            or "ddtrace.contrib.django.TraceExceptionMiddleware"
            not in settings.MIDDLEWARE_CLASSES
        )
    ...
    def test_autopatching_twice_middleware(self):
        ...
        found_mw = settings.MIDDLEWARE.count(
            "ddtrace.contrib.django.TraceExceptionMiddleware"
        )
        eq_(found_mw, 1)
